...
public:
    ...
    void dump()
    {
        Pre_visit_node(this->root_);
    }

    int GetMax()
    {
        rb_tree_node<T>*root=root_;
        rb_tree_node<T>*cur=NULL;
        while (root!=NULL)
        {
            cur=root;
            root=root->r_child_;
...
private:
    ...
    void Pre_visit_node(const_node_pointer p)
    {
        if (nullptr == p) { return; }
            cout << p->data_ ;
            if (p->color_==RED)cout<<"(R) ";
            else cout<<"(B) ";
            Pre_visit_node(p->l_child_);
            Pre_visit_node(p->r_child_);
    }
};
int main() {
    int N;cin>>N;
    ...
    if(MyRBTree.root_==nullptr)cout<<"Null";
    else MyRBTree.dump();
    return 0;
}
